...
data = {
    ...
    "Lig": ["Süper Lig", "Premier League", "La Liga", "Süper Lig", "Bundesliga", "Süper Lig"],
    "Tarih": [
        "07 Nisan 2026 - 21:00", 
        "08 Nisan 2026 - 22:00", 
        "09 Nisan 2026 - 23:00", 
    ...
    "1xBet Ev": [2.15, 1.68, 1.92, 2.45, 1.75, 1.95],
    "1xBet Beraberlik": [3.50, 4.20, 3.80, 3.40, 4.10, 3.60],
    "1xBet Deplasman": [3.10, 4.75, 3.95, 2.75, 4.50, 3.80],
    "AI Kazanma Olasılığı (%)": [59, 64, 57, 51, 68, 55],
    "Value Bet": ["✅ EVET (+14%)", "✅ EVET (+9%)", "❌ Hayır", "⚠️ Şüpheli", "✅ EVET (+11%)", "✅ EVET (+7%)"],
    "Uyarı": ["", "Ani Oran Düşüşü!", "", "Whale Alert (Under)", "", "Form avantajı evde"]
}
...
# Basit ve güvenli renk stili (eski hata düzeltildi)
def color_probability(val):
    try:
        ...
        if val >= 65:
            ...
            return 'background-color: #FFB347; color: black'
    except:
        return ''

# Tabloyu stil ile göster
styled_df = df.style.applymap(color_probability, subset=['AI Kazanma Olasılığı (%)'])
...
with col1:
    ...
    st.success("**Toplam Oran: 6.32**   →   100 TL bahis → **632 TL** potansiyel")
    
    if st.button("📋 Kuponu Kopyala"):
        st.toast("✅ Kupon kopyalandı! 1xBet’e yapıştırabilirsin.")

with col2:
    st.subheader("🔥 AI Uyarıları")
    st.warning("**Man City maçı:** Oran ani düştü → **Value fırsat!**")
    st.info("**Galatasaray maçı:** Kadro ve xG üstünlüğü ev sahibinde (%59)")
    st.error("**Trabzonspor maçı:** Under tarafında yüksek hacim var (Whale hareketi)")

# ====================== ALT BİLGİ ======================
st.caption("Bu demo versiyondur. Gerçek zamanlı oranlar ve daha güçlü AI için API entegrasyonu yapılacaktır.")
st.markdown("---")
